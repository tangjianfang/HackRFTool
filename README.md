...
| 当前状态 | **M0–M5 全部完成（2026-09-05）**：v0.1 频谱+瀑布（[M1](docs/m1-交付记录.md)）· v0.2 信道监测/统计/CSV（[M2](docs/m2-交付记录.md)）· v0.3 抓包离线链路（[M3](docs/m3-交付记录.md)）· v0.4 全频段扫描（[M4](docs/m4-交付记录.md)）· v0.5 实时抓包 UI + ESB 协议解帧（[M5](docs/m5-交付记录.md)） |
